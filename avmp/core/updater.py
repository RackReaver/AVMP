...
def main(vuln_db_filepath, config):
    ...
    try:
        jiraAPI = JiraToolsAPI(
            config["creds"]["jira"]["server"],
    ...
    for num, ticket in enumerate(tickets):
        ...
        logged_work = jiraAPI.log_work(ticket[0], "5m", comment=COMMENT)

        # Attempt logging time to root ticket if original ticket does not allow
        if root_ticket != "" and logged_work == False:
            root_comment = "{} - {}".format(ticket[0], COMMENT)
            jiraAPI.log_work(root_ticket, "5m", comment=root_comment)
