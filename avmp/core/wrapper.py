...
def static(app):
    ...
    logging.info(
        '{} used "{}" to create parent ticket "{}" successfully in Jira'.format(
    ...
    logging.info("Successfully created parent ticket ({})".format(parent_ticket))
    ...
    for key in app.process_config["sub_tasks"]:
        ...
        if "parent" not in app.process_config["sub_tasks"][key]:
            app.process_config["sub_tasks"][key]["parent"] = {"key": parent_ticket}

        child = app.jiraAPI.create(app.process_config["sub_tasks"][key])
        logging.info(
            '{} used "{}" to create child ticket "{}" under "{}" successfully in Jira'.format(
                socket.gethostname(),
                app.config["creds"]["jira"]["username"],
                child,
                parent_ticket,
            )
        )
...
def dynamic(app):
    ...
    if app.tenAPI.check_scan_in_progress(app.process_config["scan_name"]) == True:
        ...
        filepath = app.tenAPI.export_latest_scan(
            app.process_config["scan_name"],
            os.path.join(os.getcwd(), "data", "scans"),
            overwrite=False,
        )
        ...
        items = TenableToolsCSV(
            filepath, min_cvss_score=app.process_config["min_cvss_score"]
        ).group_by("Plugin ID")
        ...
        logging.info(
            'Creating tickets for "{}"'.format(app.process_config["scan_name"])
        )
        ...
        for ticket in tickets.values():
            ...
            if (
                ticket_counter < app.process_config["max_tickets"]
                or app.process_config["max_tickets"] == 0
            ):
                ...
                if (
                    len(app.config["types"]) > 0
                    and data["project"]["key"] in app.config["types"]
                ):
                    missing_fields = []
                ...
                dups = db.get_all_tickets_by_plugin_id(ticket["Vuln Data"]["Plugin ID"])

                # TODO: Add abilty to open ticket for new IP's and link to existing vuln ticket.

                if (
                    len(dups) == 0
                    or app.process_config["allow_ticket_duplication"] == True
                ):
                    if len(dups) != 0:
                        logging.info(
                            "Plugin ID ({}) has an open ticket. Creating duplicate...".format(
                    ...
                    db.add_ticket(
                        current,
                        ticket["Vuln Data"]["Plugin ID"],
                        app.process_config["default_ticket_status"],
                        list(ip_list),
                    )
                    ...
                    if app.process_config["root_ticket"] != "":
                        try:
                            app.jiraAPI.link(
                                app.process_config["root_ticket"],
                                current,
                            ...
                            logging.error(
                                "Failed to link {} to root ticket {}".format(
                                    current, app.process_config["root_ticket"]
                                )
                            )
                else:
                    logging.info(
                        "Plugin ID ({}) has an open ticket. Skipping...".format(
                            ticket["Vuln Data"]["Plugin ID"]
                        )
                    )
...
def jira_log_time(app, ticket):
    ...
    logged_work = app.jiraAPI.log_work(
        ticket,
        app.process_config["time_saved_per_ticket"],
        comment=app.process_config["time_saved_comment"],
    )

    if (
        "root_ticket" in app.process_config
        and app.process_config["root_ticket"] != ""
        and logged_work == False
    ):
        root_comment = "{} - {}".format(
