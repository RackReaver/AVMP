...
class App:
    ...
    def tenAPIcon(self):
        ...
        if (
            "access_key" in self.config["creds"]["tenable"]
            and "secret_key" in self.config["creds"]["tenable"]
        ):

            self.tenAPI = TenableToolsAPI(
                self.config["creds"]["tenable"]["access_key"],
    ...
    def jiraAPIcon(self):
        ...
        if (
            "server" in self.config["creds"]["jira"]
            and "username" in self.config["creds"]["jira"]
            and "password" in self.config["creds"]["jira"]
        ):

            self.jiraAPI = JiraToolsAPI(
                self.config["creds"]["jira"]["server"],
