...
class TenableToolsAPI:
    ...
    def get_scan_info(self, scan_name):
        ...
        for scan in scan_list:
            if scan["name"] == scan_name:
                latest_scan_uuid = next(self.tio.scans.history(scan["id"]))["scan_uuid"]
                return self.tio.scans.info(scan["id"], latest_scan_uuid)

    def export_latest_scan(
        self, scan_name, folder, overwrite=None, export_format="csv"
    ):
        ...
        history_id, scan_date = self._get_latest_history_id(str(scan_meta_data["id"]))
...
def build_jira_description(ticket):
    ...
    jira_description += "*({})*\n".format(ticket["Vuln Data"]["Plugin ID"].strip())
    ...
    for host in ticket["Hosts"].values():
        ...
        code_str = "{code}"
        jira_description += "{}{}{} |".format(code_str, host["Plugin Output"], code_str)
        jira_description += "\n"
...
def zulu_to_mdy(zulu_date):
    ...
    """
    return datetime.strptime(zulu_date[:19], "%Y-%m-%dT%H:%M:%S").strftime(
        "%m-%d-%Y %I:%M %p"
    )
